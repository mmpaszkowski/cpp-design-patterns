...
namespace dp {
    ...
    constexpr HeroFactoryImpl::HeroFactoryImpl(std::unique_ptr<Mage> &&mage,
                                               ...
                                               std::unique_ptr<Beast> &&beast) noexcept:
            mage_(std::move(mage)),
            warlord_(std::move(warlord)),
            beast_(std::move(beast)) {}

    constexpr HeroFactoryImpl::~HeroFactoryImpl() = default;
    constexpr std::unique_ptr<Mage> HeroFactoryImpl::createMage() const { return mage_->clone(); }
    constexpr std::unique_ptr<Warlord> HeroFactoryImpl::createWarlord() const { return warlord_->clone(); }
    constexpr std::unique_ptr<Beast> HeroFactoryImpl::createBeast() const { return beast_->clone(); }
